...
db "FDC MB8877A  "      ;comment

fdcjmp  dw fdcinp,fdcout,fdcact,fdcmof0
fdcmof  jp fdcmofx
        db 32*0+2       ;bit[0-4]=driver ID (2=national), bit[5-7]=storage type (0=FDC)
        ds 4
fdcslt  ds 3
...
fdc_status      equ #7fb8   ;FDC status register                ##!!##seems to work both?? (7fb8 or 7f80)##
fdc_command     equ #7fb8   ;FDC command register
fdc_track       equ #7fb9   ;FDC track register
fdc_sector      equ #7fba   ;FDC sector register
fdc_data        equ #7fbb   ;FDC data register

fdc_drive       equ #7fbc   ;drive (,side) and motor select

fdc_drq         equ #7fbc   ;drq (bit7), busy (bit6)
...
;### FDCINP -> read sectors (512b)
;### Input      A=device (0-7), IX=logical sector number, B=number of sectors, (stobnkx)=banking config
;### Output     CF=0 -> ok
;###            CF=1 -> A=error code (...)
;### Destroyed  AF,BC,DE,HL,IX,IY
...
fdcinp4 call fdcred
        ...
        dec b
        jp z,bnkdofx        ;all sectors have been loaded -> finished
        ld a,6
        ld (fdcinpr),a
        inc h:inc h
...
;### FDCEXE -> prepare FDC for execution phase (sector read or sector write)
;### Input      A=sector, B=command, (stobnkx)=banking config, HL=address
;### Output     PC=(SP)   -> ok, HL=converted data address
